...
export class AiAgentManagement1756080000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      CREATE INDEX IF NOT EXISTS idx_agents_business_id_status
        ON agents(business_id, status);

      CREATE UNIQUE INDEX IF NOT EXISTS uq_agents_business_lower_name_non_archived
        ON agents (business_id, lower(name))
